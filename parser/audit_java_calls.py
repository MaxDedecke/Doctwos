...
def main():
    ...
    results = [parse_java_file(path.read_text(encoding='utf-8'), path.relative_to(args.root).as_posix())
    ...
    print(json.dumps({
        ...
        'reasons': dict(Counter(edge.meta.get('resolution_reason', '<missing>') for edge in calls)),
        'super_calls': [{
            'path': result.path, 'line': edge.src_start_line, 'target': edge.dst_name,
            'resolution': edge.resolution, 'reason': edge.meta.get('resolution_reason'),
        } for result in results for edge in result.edges if edge.type == 'CALLS'
            and edge.meta.get('receiver') == 'super'],
    }, indent=2))
