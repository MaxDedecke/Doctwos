...
def test_source(db_session):
    ...
    source = KnowledgeSource(
        name="Test-Dokument", type="local_document", project_id=project_id, team_id=team_id
    )
    ...
    entity_a = CodeEntity(
        project_id=project_id, source_id=source.id, file_path="a.py", name="EntityA", type="generic"
    )
    entity_b = CodeEntity(
        project_id=project_id, source_id=source.id, file_path="b.py", name="EntityB", type="generic"
    )
...
@pytest.mark.anyio
async def test_duplicate_content_chunks_fall_back_to_pending_instead_of_wrong_rewire(
    ...
    await reindex_chunks_preserving_links(
        ...
        build_chunk=_build_chunk(source),
    ...
    chunk_ids = [
        ...
        for row in db_session.execute(
            text("SELECT id FROM document_chunks WHERE source_id = :sid ORDER BY id"),
            {"sid": source.id},
        ).all()
    ]
...
@pytest.mark.anyio
async def test_unique_content_still_rewires_correctly_when_other_chunk_is_duplicated(
    db_session, test_source
):
    ...
    db_session.expire_all()
    all_chunks_after = (
        db_session.query(DocumentChunk).filter(DocumentChunk.source_id == source.id).all()
    )
    new_unique_chunk_id = next((c.id for c in all_chunks_after if c.content == unique_text), None)
    assert new_unique_chunk_id is not None, (
        "Chunk mit dem eindeutigen Inhalt nach dem Reindex nicht gefunden"
    )
