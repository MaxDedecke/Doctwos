...
async def test_compute_entity_links_stale_lock_override(db_session, test_project):
    ...
    stale_run = LinkBuilderRun(project_id=project_id, task_type="entity_links", status="completed")
    ...
    current_run = LinkBuilderRun(project_id=project_id, task_type="entity_links", status="pending")
