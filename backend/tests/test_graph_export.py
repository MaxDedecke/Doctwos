...
def _fixture_graph(db, project_id, team_id):
    source = KnowledgeSource(
        name="graph-export-source",
        type="Git",
        url="https://example.test/export.git",
        branch="main",
    ...
    entity = CodeEntity(
        project_id=project_id,
        source_id=source.id,
        file_path="EXPORT.CBL",
        name="EXPORT-PROGRAM",
        qualified_name="EXPORT-PROGRAM",
        type="program",
        start_line=1,
        end_line=10,
    )
...
def test_graph_csv_export_contains_the_approved_link_as_an_edge_row(
...
def test_graph_graphml_export_contains_matching_nodes_and_edge(
    client, db_session, test_project, test_team
):
    source, entity, chunk, link = _fixture_graph(db_session, test_project, test_team)
    try:
