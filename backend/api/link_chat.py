        ...
        results.append(
            {
                ...
                "type": s.type,
                "name": s.name,
                "item_count": db.query(DocumentChunk)
                .filter(DocumentChunk.source_id == s.id)
                .count(),
                "source_id": s.id,
            }
        )
...
async def send_link_chat_message(
    ...
    async def event_generator():
        ...
        results = (
            chunk_query.order_by(DocumentChunk.embedding.cosine_distance(query_embedding))
            .limit(6)
            .all()
        )
        ...
        for i in range(len(results)):
            for j in range(i + 1, len(results)):
                ...
                if (a.source_id and a.source_id == b.source_id) or (
                    a.project_id and a.project_id == b.project_id
                ):
                    continue
                ...
                suggestions.append(
                    {
                        ...
                        "source_a_source_type": meta_a.get("source_type")
                        or chunk_source_label(a, db),
                        "source_a_chunk_id": a.id,
                        "source_a_url": meta_a.get("url"),
                        "source_b_title": meta_b.get("title") or b.file_path,
                        "source_b_source_type": meta_b.get("source_type")
                        or chunk_source_label(b, db),
                        "source_b_chunk_id": b.id,
                        "source_b_url": meta_b.get("url"),
                        "score": 0.85,
                        "context": "Diese Dokumente sind semantisch verwandt.",
                    }
                )
        ...
        assistant_msg = ChatMessage(
            ...
            role="assistant",
            content=answer,
            metadata_json={"type": "link_assistant", "suggestions": suggestions[:3]},
        )
...
def confirm_suggested_links(
    ...
    for data in links_data:
        if not (
            is_side_visible(
                data.get("source_a_type", "document"),
                data.get("source_a_entity_id"),
                data.get("source_a_chunk_id"),
                user,
                db,
            )
            and is_side_visible(
                data.get("source_b_type", "document"),
                data.get("source_b_entity_id"),
                data.get("source_b_chunk_id"),
                user,
                db,
            )
        ):
            raise HTTPException(
                status_code=403, detail="Kein Zugriff auf eine der verknüpften Quellen"
            )
    ...
    db.commit()
    return [serialize_knowledge_link(link) for link in created_links]
