...
| `OLLAMA_NUM_CTX` | Context window (tokens) Ollama is told to use for chat, the agent tool loop and the Code Compliance Checker. Default `8192` (O-168). Without this, Ollama silently falls back to its own small built-in default and truncates from the front once it's full — the model can lose the system prompt, retrieved knowledge or early agent tool-loop results without any error. Raise it on a delivery host with RAM/VRAM to spare, especially if agent-mode answers (multi-step tool use across up to 8 turns) seem to "forget" earlier steps; a bigger context window costs more memory per request, so don't raise it past what the host in "Hardware sizing" above actually has headroom for. |
| `INFERENCE_MAX_CONCURRENCY` / `INFERENCE_CHAT_RESERVE` / `INFERENCE_BATCH_RESERVE` | Shared per-endpoint inference slots coordinated through Valkey/Redis (O-258). Defaults: 4 total, 1 reserved for chat and 1 for imports/linking, so neither class can occupy all capacity. The backend and parser workers must use the same values. Tune against measured model-server capacity; `INFERENCE_CHAT_WAIT_SECONDS` (30) bounds interactive queueing and `INFERENCE_BATCH_WAIT_SECONDS` (900) lets imports wait for capacity rather than fail immediately. |
...
| `OIDC_DEFAULT_TEAM` / `OIDC_TEAM_MAPPING` / `OIDC_ADMIN_ROLES` / `OIDC_ROLES_CLAIM` / `OIDC_GROUPS_CLAIM` | **Optional (O-164)** — Automatic team assignment and superuser role mapping for SSO logins. `OIDC_DEFAULT_TEAM` assigns every SSO user to a team (e.g. `Default Team`). `OIDC_ADMIN_ROLES` is a comma-separated list of IdP roles mapped to `superuser` (e.g. `doctus-admin`). `OIDC_TEAM_MAPPING` maps IdP groups/roles to Doctus teams as JSON (`'{"dev": "Entwicklung"}'`) or comma list. |
