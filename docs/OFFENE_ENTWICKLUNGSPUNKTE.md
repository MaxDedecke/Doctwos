...
| O-038 | Frontend / Chat-Sessions | **Neu:** eine Chat-Session soll nicht mehr nur mit der ersten Chat-Nachricht beginnen, sondern auch, sobald eine zweite View geöffnet wird (z. B. Graph-View + Code-View), selbst wenn der Chat nie benutzt wurde. Aktuell ist ein Befund, der nur über mehrere Views ohne Chat-Nutzung entsteht, nicht teilbar oder konservierbar, weil ohne Chat-Nachricht keine Session existiert. Vorschlag: neues Speicher-Icon in der Header-Bar, das nur erscheint, wenn der Chat leer ist UND eine zweite View geöffnet ist; Klick öffnet einen Dialog zur Namensvergabe, danach erscheint die Session unter diesem Namen in der Sidebar. | Konzept zur Session-Erzeugung ohne Chat-Nachricht ausarbeiten (wann genau „beginnt" die Session serverseitig, Verhältnis zum bestehenden Session-Anlage-Pfad) und umsetzen. | Bestehender Chat-Session-/Sidebar-Mechanismus |
