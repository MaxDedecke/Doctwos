...
| ID | Bereich | Punkt | Status / nächste Aktion | Abhängigkeit |
|---|---|---|---|---|
| O-041 | Sicherheit / Authentifizierung | Login ist ein reines lokales Passwort-Verfahren (`backend/api/auth.py`, Argon2id) — im Backend existiert kein OIDC-/SAML-/LDAP-Code (einziger Treffer für „oidc" im gesamten Backend ist ein Alembic-Migrationsname). `docs/deployment-customer.md` beschreibt dagegen bereits eine Anleitung, beim Kunden Issuer-URL/Client-ID/Client-Secret für einen OIDC-Login abzufragen — Doku und Code widersprechen sich. Ohne echtes SSO ist Doctus für Konzernkunden (Fujitsu/DRV-Format) faktisch nicht einsetzbar, nur Pilotbetrieb mit lokalen Testkonten (`testuser`/`testpass`). | OIDC-Client-Flow (z. B. Authlib) gegen einen Kunden-IdP (Keycloak/Entra ID/Okta) implementieren, lokales Passwort-Login als Fallback für Admin-Bootstrap behalten. Deployment-Doku danach wieder mit dem Code abgleichen. | Höchste Priorität — Vertriebsblocker für Enterprise-Kunden |
| O-042 | Konnektoren | Kein Konnektor für mainframe-native Quellcode-Verwaltung (Endevor, ChangeMan, Micro Focus/PVCS, IBM RTC/CMVC) oder direkten Zugriff auf z/OS-PDS-Datasets. `parser/connectors/git.py` deckt nur Git-Bestände ab (providerneutraler Bare-Mirror-Clone, funktioniert daher zwar mit GitHub/GitLab/Azure Repos, nicht nur Bitbucket — aber eben nur mit Git). COBOL-Bestände, die noch nicht nach Git migriert sind, lassen sich damit gar nicht anbinden. | Mit Vertrieb/Kunden klären, welches SCM im Zielsegment tatsächlich vorherrscht, bevor ein bestimmter Konnektor gebaut wird — sonst Fehlinvestition. | Fachliche Priorisierung nötig |
