...
| | Wissensgraph (`GET /graph`, `/graph/neighborhood`) | Process-/Call-Graph (`GET /callgraph/focus`) | RAG-Graph-Erweiterung (`graph_retrieval.expand_chunks_with_graph`) |
|---|---|---|---|
| **Zweck** | Visualisierung: systemübergreifende Beziehungen zwischen Code-Dateien und quellenspezifischen Wissensressourcen | Visualisierung: Code-Struktur, Aufrufe und Prozesspfade mit technischen Kantentypen | Kein sichtbarer Graph — reichert den Chat-Kontext eines Vektor-Treffers an |
| **Knoten** | Code-Dateien plus bestätigte `EntityDocLink`-/`KnowledgeLink`-Endpunkte. Dokumentressourcen werden pro Quelle als Seite, Issue oder Datei identifiziert; Chunks bleiben Belege auf Kanten. | `CodeEntity` (BFS über 0–5 `hops` ab einer Wurzel-Entity) | `CodeEntity`, ermittelt über Datei+Zeilen-Überlappung mit dem Vektor-Treffer |
| **Kanten** | `CodeEdge`s werden pro Quelldateipaar gerichtet zusammengefasst; `EntityDocLink`s laufen von der dokumentierenden Ressource zur Code-Datei und tragen Code- sowie Dokumentfundstelle; `KnowledgeLink`s behalten ihre gespeicherte Richtung. Standardmäßig nur `status=approved`. | `CodeEdge` mit exaktem Typ und Richtung + **synthetische** `CONTAINS`-Kante aus `CodeEntity.parent_id` (nur Vorfahren nachgezogen, nie Kinder — sonst würde ein Paragraph-Fokus den Knotendeckel mit Datenfeldern sprengen) | Aufgelöste `CodeEdge`s mit `type ∈ {COPY, CALL, CALLS, INSTANTIATES, EXTENDS, IMPLEMENTS, USES_TYPE, READS, WRITES}` ab den getroffenen Entities |
