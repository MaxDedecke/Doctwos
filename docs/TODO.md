...
| Rang | ID | Ergebnis | Abhängigkeit / Abnahme |
|---:|---|---|---|
| 73 | O-338 | Introduce vLLM next to Ollama: Inferenz-Backend für Chat und Tool-Calling zertifizieren. | Validiere vLLMs OpenAI-kompatiblen Endpunkt (`/v1/chat/completions`) neben Ollama für Qwen 2.5 Coder (14B/32B/72B) inkl. nativer Tool-Aufrufe (`tools`, `tool_choice`). Healthcheck-Adapter für `/health` und `/v1/models` in die Profil-Erreichbarkeitsprüfung integrieren; Fehlerverhalten bei GPU-Memory-Exhaustion abfangen. |
...
### Keplair Core Substrat, Data Lineage & Agentic Governance (Zukunftssicherung)

| Rang | ID | Ergebnis | Abhängigkeit / Abnahme |
|---:|---|---|---|
| 77 | O-342 | Sub-atomare Data-Lineage auf Feldebene (COBOL `REDEFINES`, Copybooks & DB-Hostvariablen). | Erweiterung des Parsers und Resolvers über reine Paragraphen-/Programmaufrufe (`CALL`/`PERFORM`) hinaus auf Datenflüsse auf Variablenebene. Verfolgung von Zuweisungen (`MOVE`, `COMPUTE`) und Speicher-Overlays (`REDEFINES`) über Modulgrenzen hinweg. Fragestellung deterministisch beantworten: *„Welche Felder/Tabellenspalten werden implizit modifiziert, wenn Feld `KTO-SALDO` in Copybook X geändert wird?“*. Regressionstest an CardDemo (`COACTUPC`/`COTRTUPC`) mit Nachweis von Byte-Offset-Genauigkeit ohne LLM-Raten. |
| 78 | O-343 | Headless Agentic Guardrail & Verification-API via MCP (Ground Truth für autonome KI-Agenten). | Bereitstellung von Keplair als Headless-Wahrheitsanker für externe Coding-Agenten (Cursor, Claude Code, bankinterne LLM-Orchestrierer). Standardisierte MCP-Tools und REST-Endpoints: `verify_proposed_change(diff)`, `get_blast_radius(symbol)`, `trace_data_flow(field)`. Ein externer Agent kann ein Refactoring simulieren; Keplair antwortet deterministisch mit dem exakten Bruch-Risiko (`PASS`/`FAIL` mit Zeilennachweisen). |
| 79 | O-344 | Automatisierter „4-Wochen Architecture Audit Sprint“-Generator (Anti-Consulting-Produkt). | Schlüsselfertige, automatisierte Generierung des Audit-Deliverables auf Knopfdruck, um 15k–25k € Sprints ohne manuellen Beratungsaufwand als Softwareprodukt zu skalieren. Endpunkt/Job zur Aggregation von: Dead-Code-Anteil, zirkulären Abhängigkeiten, CICS/IMS-Kopplungsgrad, unaufgelösten dynamischen Aufrufen und Risiko-Top-10. Export als eigenständiges, klickbares Offline-HTML-Dashboard und CISO-taugliches Executive-PDF. |
| 80 | O-345 | DORA / BaFin BAIT Compliance- & Audit-Trail-Export (Revisionssichere IKT-Risikoakte). | Kryptografisch signierter Export (SHA256 über Git-Commit, Graph-Snapshot und Abhängigkeitsmatrix). Formaler Nachweis für interne Revision und BaFin/BSI-Prüfer: Welche Altsysteme greifen auf welche kritischen Datenfunktionen zu? Wandelt den Kaufanlass von „nice-to-have Produktivitätsgewinn“ in „gesetzlich vorgeschriebene Risikoprävention“ (Haftungsbefreiung) um. |
| 81 | O-346 | Positionierungs- & Narrativ-Schärfung im Pitch Deck (`keplair.md` & HTML-Deck). | Refactoring der Außendarstellung und Kernbotschaft zur Abgrenzung von herkömmlichen KI-Coding-Tools. Primäres Wording weg von: *„Offline-KI-Assistent / Legacy Code Explorer“* (Klingt nach Feature und birgt Redundanz-Verdacht) hin zu: *„The Deterministic Code Intelligence & Safety Gate for Regulated Core Systems“*. Betonung des Dual-Core-Prinzips: Mathematische Wahrheit (AST/Graph) als Fundament, sichere lokale Inferenz als Interface. |
