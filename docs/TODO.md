...
| 17 | O-317 | Datei- und Projektfokus im Eval-Retrieval erzwingen. | Mehrere Eval-Fragen erhielten trotz explizitem Dateinamen Entitäten aus einem anderen COBOL-Programm (`PAUDBUNL`, `COPAUS0C`) bzw. fachfremde Java-Dateien. Dateipfad/Programmname als harte Retrieval- und Tool-Filter verwenden; Treffer außerhalb des Filters verwerfen oder als nicht belegt melden. Regression mit J1/J2/C1/C4/C6. |
| 18 | O-318 | Quellenkonsistenz vor Chat-Antwort validieren. | Vor dem finalen SSE-Answer prüfen, dass behauptete Datei, Zeilen und Kanten in den tatsächlich gelieferten Tool-Ergebnissen vorkommen. Bei fehlender Zielquelle keine plausible Ersatzdatei als Beleg verwenden; stattdessen explizite Index-/Parserlücke ausgeben. |
| 19 | O-319 | Zielpfad aus der Nutzerfrage vor dem allgemeinen Repository-Bootstrap extrahieren. | **Teilweise umgesetzt:** Backtick-Pfade, Dateinamen und Programm-IDs werden jetzt vor dem Bootstrap als Filter verwendet; J1 und C1 öffnen dadurch die richtige Primärdatei. Exakte Entity-Suche für J2/J3/J4/C3 sowie Regression bleiben offen. |
| 20 | O-320 | COBOL-Dateisuche quellenweit statt verzeichnislokal machen. | **Teilweise umgesetzt:** quellenweite Suche und eindeutige Pfadauflösung sind aktiv; `COTRTLIC.cbl` wird in C6 korrekt geöffnet. Journal-/Dateilisting-Konsistenz und vollständige Tiefensuche bleiben abzunehmen. |
| 21 | O-321 | Exakte Entity-Auflösung vor `trace_call_flow` erzwingen. | Tool-Aufrufe dürfen nicht mit einem zufällig gebootstrappten Entity gestartet werden. Bei mehreren Namen zuerst vollständigen Qualified Name/Pfad auflösen und den gewählten Treffer samt Unsicherheit anzeigen. Abnahme: C1 trace't `COPAUA0C.MAIN-PARA`, nie `PAUDBUNL.MAIN-PARA`. |
| 22 | O-322 | Eval-Fragen mit Ground-Truth-Datei regressionssichern. | Für J1/J2/J3/J4/J5 und C1–C6 automatisierte Tests anlegen, die Primärdatei, relevante Zeilen, aufgelöste/unaufgelöste Kanten und das Verhalten bei fehlender Quelle prüfen. Eine plausible Antwort aus einer Ersatzdatei muss den Test fehlschlagen lassen. |
| 23 | O-323 | Chat-SSE-Telemetrie für Eval und Betrieb vervollständigen. | Pro Anfrage `request_received`, erster Tool-Call, First-Token, Tool-Ende, Modell-Ende und `message_saved` mit monotoner Zeit erfassen; Antwortzeit, First-Token, Tool-Anzahl und Retrieval-Wartezeit im Ergebnisprotokoll ausgeben. |
...
| 14 | O-308 | Java-Records, Parameter, relevante lokale Variablen, lokale/anonyme Klassen, Lambdas und Methodenreferenzen adressierbar machen. | Teilweise umgesetzt: alle genannten Elemente erhalten quellennahe Java-Entities und stabile Qualified Names; Auflösung ihrer fachlichen Beziehungen bleibt bei O-309/O-310. Graphübersicht größenbegrenzt halten. |
