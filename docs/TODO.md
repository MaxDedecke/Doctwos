...
| 5 | O-309 | Java-Aufrufe über Parameter, lokale Variablen, Felder, Vererbung und Interfaces belegbar auflösen. | Teilweise umgesetzt: Receiver-Typen aus Parametern, lokalen Variablen/Feldern (inkl. `var`-`new`), explizite Vererbung und Interfaces werden mit Ziel-Qualified-Name und Receiver-Evidenz aufgelöst; Mehrdeutigkeiten/dynamische Ausdrücke bleiben offen. Referenztests und Auditfelder sind ergänzt, die Abnahme am Zielbestand steht aus. |
